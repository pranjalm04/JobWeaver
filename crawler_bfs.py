...
from crawl4ai import CrawlResult, AsyncWebCrawler, CrawlerRunConfig, MemoryAdaptiveDispatcher
...
class BFSCrawl():
    ...
    def __init__(
        ...
        self.JOB_PATTERNS = re.compile(
            ...
            r'('
            r'(?:career|job|position|employment|search|apply|opportunit)(?:s|y|ies)?'  # matches "career", "careers", "opportunity"
    ...
    async def _arun_batch(
        ...
        while current_level and not self._cancel_event.is_set():
            ...
            for i in range(0, len(urls_depth), max_concurrent):
                ...
                for (url, depth), result in zip(batch, result_batch):
                    if isinstance(result, Exception):
                        ...
                        # score,debug_info,is_job_listing,job_listing_metadata=self.link_discovery(result, url, depth, visited, next_level)
                        # results_with_discovery.append((score,debug_info,is_job_listing,job_listing_metadata))
                        task = asyncio.create_task(self.link_discovery(result, url, depth, visited, next_level))
            ...
            await self.clean_up_sessions(session_id)
            current_level = filtered_next_level
            print(f"----------------FINISHED CRAWLING URLS AT LEVEL {depth_bfs}--------------------------------")
            depth_bfs+=1

        results_sorted_by_highest_score = sorted(results, key=lambda x: (-x['job_listing_score'], len(x["url"])))

        for r in results_sorted_by_highest_score:
         print(f"{r['url']}, {r['job_listing_score']},{r['debug_info']}")
        del results
        # await self.clean_up_sessions(session_ids)
    ...
    async def clean_up_sessions(self,*sessions):
        for session in sessions:
            await asyncio.create_task(self.crawler.crawler_strategy.kill_session(session))
