...
LISTING_KEYWORDS_TITLE_H1 = [
    ...
    "listing", "search jobs", "find jobs", "job openings","job listings" "job vacancies", "employment opportunities"
]
...
PAGINATION_CLASSES = ["pagination", "pager", "page-numbers", "page-list", "paging", "pagination-container","paginator"]
...
JOB_TITLE_FRAGMENTS = [
    "analyst", "specialist","engineer","manager", "coordinator",'junior',"assistant","associate","advocate","director", "assistant", "designer", "consultant","surgeon", "nurse","therapist","radiology","psychiatrist","support worker","councellor","technologist","scientist"
]
...
#     #                     ("div", "Career"), ("tr", "job"), ("article","item"),("article", "career"),("div","position"),("section","job")]  # Added table row
#
#     potential_individual_items = []
#
#     # Look for common list item/container tags that might be individual job wrappers
#     possible_item_tags = soup.find_all(['div', 'li', 'article', 'tr','section'],
#                                        limit=200)  # Increase limit to find more candidates
#
#     for item_candidate in possible_item_tags:
#         # Check for a primary link within this candidate element
#         links_in_candidate = item_candidate.find_all('a', href=True, limit=5)  # Limit search for links within
#
#         # Criteria 1: Should contain at least one link
#         if not links_in_candidate:
#             continue
#
#         # Criteria 2: Check if at least one link looks like a job title link
#         is_likely_job_item = False
#         for link in links_in_candidate:
#             link_text = link.get_text(strip=True)
#             href = link.get('href', '')
#
#             # Basic link validation (similar to fallback check, but applied to potential item)
#             if 1 < len(link_text.split()) < 10 and re.search(r'[a-zA-Z]', link_text) and not href.startswith(
#                     ('http', '#', 'javascript:', 'mailto:')) and len(href) > 1:
#                 link_text_lower = link_text.lower()
#                 # Check if link text contains job title fragments
#                 if any(frag in link_text_lower for frag in JOB_TITLE_FRAGMENTS):
#                     is_likely_job_item = True
#                     break  # Found a likely job title link, check other criteria
#
#         if is_likely_job_item:
#             # Criteria 3: Check for job item indicators within the candidate's text
#             item_text = item_candidate.get_text(" ", strip=True).lower()
#             has_job_keyword = any(ind in item_text for ind in JOB_ITEM_INDICATORS)
#
#             # Criteria 4: Optional - Check for absence of multiple *other* significant links
#             # This is harder to make robust universally, so might be skipped initially.
#             # You could count other links within the item_candidate and set a threshold.
#
#             if has_job_keyword:# Example: Require a likely title link AND a job keyword
#                 potential_individual_items.append(item_candidate)
#
#     # Now, analyze potential_individual_items for repetition and validity
#     # (Similar to the previous logic, but applied to the list of potential *individual* items)
#
#     unique_individual_items = list({item: True for item in potential_individual_items}.keys())
#
#     if len(unique_individual_items) >= 2:
#         # Check if these items contain typical job info (already done in the loop, but could add more checks here)
#         # Analyze similarity of structure (more advanced)
#
#         # For simplicity, let's assume finding multiple unique items with the above criteria is strong
#         valid_items_count = len(unique_individual_items)
#
#         if valid_items_count >= 2:
#             item_score = 4.0 * min(valid_items_count / 5.0, 1.0)
#             score += item_score
#             debug_info.append(f"(+{item_score:.1f}) Found {valid_items_count} repeating individual job items.")
#             potential_item_count = valid_items_count # You can use this for the fallback check condition
#
...
def check_job_listing_heuristics(html_content, url=None):
    ...
    pagination_score:float = 0.0
    # Check for specific classes on common tags
    pagination_elements = soup.find_all(['nav', 'div', 'ul','li'], class_=lambda x: x and any(pc.lower() in x.lower() for pc in PAGINATION_CLASSES), limit=5)
    if pagination_elements:
        pagination_score = 1.5 # High confidence if specific class found
        pagination_found = True
        debug_info.append(f"(+1.5) Found pagination element with specific class ({[e.get('class') for e in pagination_elements]})")
    ...
    common_selectors = [("div", "job"), ("li", "job"), ("article", "job"), ("div", "result"), ("li", "result"),
                        ("div", "item"), ("div", "card"),
                        ("div", "Career"), ("tr", "job"), ("div","row"),("article","item"),("article", "career"),("div","position")]  # Added table row
    ...
    #         has_job_keyword = any(ind in item_text.lower() for ind in JOB_ITEM_INDICATORS)
    ...
    if potential_item_count < 3:
        ...
        for link in all_links:
             ...
             href = link.get('href', '')
             # Basic check: seems like title, not a simple nav link, points relatively locally
             if 1 < len(link_text.split()) < 100 and not href.startswith(('http', '#', 'javascript:', 'mailto:')) and len(href) > 1:
                  link_text_lower = link_text.lower()
                  # Check if link text contains job title fragments
